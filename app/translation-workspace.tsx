...
type Copy = {
  ...
  noTranscription: string; literal: string; conversational: string; bold: string; alternative: string; more: string;
  moreGenerating: string; moreLimit: string; askTitle: string; askPlaceholder: string; askSend: string; asking: string;
...
function variantLabel(style: TranslationVariant["style"], t: Copy): string {
  return style === "literal" ? t.literal : style === "conversational" ? t.conversational : style === "bold" ? t.bold : t.alternative;
}
...
export function TranslationWorkspace({ locale, defaultLanguage, account, onRequireAccount, onChallengeRequired, onUseInUnderstand, onUseInCreate }: Props) {
  ...
  const renderSource = () => <aside className="translation-source-card" aria-labelledby="translation-source-title">
    ...
    <div className="source-tabs" role="tablist" aria-label={t.title}>
      <button type="button" role="tab" aria-selected={inputMode === "file"} className={inputMode === "file" ? "active" : ""} onClick={() => { setInputMode("file"); setError(""); }} disabled={isTranslating || isGeneratingMore}>{t.upload}</button>
      <button type="button" role="tab" aria-selected={inputMode === "text"} className={inputMode === "text" ? "active" : ""} onClick={() => { setInputMode("text"); setError(""); }} disabled={isTranslating || isGeneratingMore}>{t.paste}</button>
    </div>
    {inputMode === "file" ? <div role="tabpanel">
      <input ref={fileInputRef} className="visually-hidden" type="file" aria-label={t.choose} accept="application/pdf,image/jpeg,image/png,image/webp,text/plain,application/rtf,text/rtf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.oasis.opendocument.text,.pdf,.jpg,.jpeg,.png,.webp,.txt,.rtf,.docx,.odt" onChange={(event: ChangeEvent<HTMLInputElement>) => chooseFile(event.target.files?.[0] ?? null)} disabled={isTranslating || isGeneratingMore} />
      {file ? <div className="translation-file-preview file-preview"><div className="file-preview-header"><div><strong>{file.name}</strong><small>{formatFileSize(file.size, targetLanguage)}</small></div><button type="button" className="secondary-button" onClick={() => { setFile(null); setResult(null); if (fileInputRef.current) fileInputRef.current.value = ""; }} disabled={isTranslating || isGeneratingMore}>{t.remove}</button></div></div> : <div className={`dropzone${isDragging ? " is-dragging" : ""}${fileError ? " has-error" : ""}`} onDragEnter={(event: DragEvent<HTMLDivElement>) => { event.preventDefault(); setIsDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setIsDragging(false); }} onDrop={(event: DragEvent<HTMLDivElement>) => { event.preventDefault(); setIsDragging(false); chooseFile(event.dataTransfer.files?.[0] ?? null); }}>
        <div className="document-icon" aria-hidden="true"><span>↔</span></div><strong>{isDragging ? t.release : t.drop}</strong><p>{t.orChoose}</p><button className="secondary-button" type="button" onClick={() => fileInputRef.current?.click()} disabled={isTranslating || isGeneratingMore}>{t.choose}</button><small>{t.allowed}</small>
      </div>}
      {fileError && <p className="input-error" role="alert">{fileError}</p>}
    </div> : <div className="text-panel" role="tabpanel"><label htmlFor="translation-text">{t.sourceLabel}</label><textarea id="translation-text" value={text} onChange={(event) => { setText(event.target.value); setError(""); setResult(null); setFollowups([]); }} maxLength={50_000} rows={12} placeholder={t.sourcePlaceholder} disabled={isTranslating || isGeneratingMore} /><small>{text.length.toLocaleString(locale === "ru" ? "ru-RU" : locale === "lv" ? "lv-LV" : "en-US")} / 50 000</small></div>}
    <button className="primary-button translation-submit" type="button" onClick={() => void submitTranslation()} disabled={isTranslating || isGeneratingMore}>{isTranslating ? t.translating : t.translate}<span aria-hidden="true">→</span></button>
