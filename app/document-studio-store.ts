...
function createStore(db: D1DatabaseLike) {
  ...
  return {
    ...
    async saveManual(userId:string,documentId:string,result:GeneratedDocument,now=Date.now()){await init();const updated=await db.prepare("UPDATE generated_documents SET result_json=?,created_at=? WHERE id=? AND user_id=? RETURNING id").bind(JSON.stringify(result),now,documentId,userId).first<{id:string}>();if(updated?.id!==documentId)throw new StudioStoreError("studio_not_found",404);return documentId;},
