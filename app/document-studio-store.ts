...
export const STUDIO_LIMITS = { free: { day: 1, month: 3, words: 3500 }, pro: { day: 30, month: 300, words: 12000 } } as const;
export const STUDIO_ASSISTANT_LIMITS = { day: 60, month: 600 } as const;
...
function createStore(db: D1DatabaseLike) {
  ...
  return {
    ...
    async reserveAssistant(userId:string,now=Date.now()){await init();await db.prepare("DELETE FROM document_studio_assistant_usage WHERE status='pending' AND updated_at<=?").bind(now-600000).run();const id=crypto.randomUUID();const x=await db.prepare(`INSERT INTO document_studio_assistant_usage(id,user_id,status,created_at,updated_at) SELECT ?,?,'pending',?,? WHERE (SELECT COUNT(*) FROM document_studio_assistant_usage WHERE user_id=? AND created_at>?)<? AND (SELECT COUNT(*) FROM document_studio_assistant_usage WHERE user_id=? AND created_at>?)<? AND (SELECT COUNT(*) FROM document_studio_assistant_usage WHERE created_at>?)<2000 RETURNING id`).bind(id,userId,now,now,userId,now-DAY,STUDIO_ASSISTANT_LIMITS.day,userId,now-30*DAY,STUDIO_ASSISTANT_LIMITS.month,now-DAY).first<{id:string}>();if(x?.id!==id)throw new StudioStoreError("studio_limit_reached",429);return id;},
