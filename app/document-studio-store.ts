...
function createStore(db: D1DatabaseLike) {
  ...
  const init = () => ready ??= (async () => {
    ...
    await db.batch([db.prepare("CREATE INDEX IF NOT EXISTS generated_documents_owner_idx ON generated_documents(user_id,created_at DESC)"), db.prepare("CREATE INDEX IF NOT EXISTS studio_usage_owner_idx ON document_studio_usage(user_id,created_at DESC)")]);
  })();
