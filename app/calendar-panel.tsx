...
function quotaMessage(locale: ProfileLanguage, quota: ReminderQuota) {
  if (quota.active >= quota.activeLimit) return locale === "ru" ? `Активно ${quota.active}/${quota.activeLimit}. Отмените одно напоминание или дождитесь его отправки.` : locale === "lv" ? `Aktīvi ${quota.active}/${quota.activeLimit}. Atceliet vienu atgādinājumu vai sagaidiet tā nosūtīšanu.` : `${quota.active}/${quota.activeLimit} active. Cancel one reminder or wait until it is sent.`;
  if (quota.weeklyUsed >= quota.weeklyLimit) return locale === "ru" ? `Использовано ${quota.weeklyUsed}/${quota.weeklyLimit} за 7 дней. Новые письма временно заблокированы.` : locale === "lv" ? `Izmantoti ${quota.weeklyUsed}/${quota.weeklyLimit} 7 dienās. Jauni e-pasti īslaicīgi bloķēti.` : `${quota.weeklyUsed}/${quota.weeklyLimit} used in 7 days. New emails are temporarily blocked.`;
  return locale === "ru" ? `Активные: ${quota.active}/${quota.activeLimit} · За 7 дней: ${quota.weeklyUsed}/${quota.weeklyLimit}` : locale === "lv" ? `Aktīvi: ${quota.active}/${quota.activeLimit} · 7 dienās: ${quota.weeklyUsed}/${quota.weeklyLimit}` : `Active: ${quota.active}/${quota.activeLimit} · 7 days: ${quota.weeklyUsed}/${quota.weeklyLimit}`;
}
function quotaError(locale: ProfileLanguage, code: string) {
  if (code === "active_reminder_limit") return locale === "ru" ? "Уже активно три напоминания. Отмените одно или дождитесь его отправки." : locale === "lv" ? "Jau ir aktīvi trīs atgādinājumi. Atceliet vienu vai sagaidiet tā nosūtīšanu." : "Three reminders are already active. Cancel one or wait until it is sent.";
  if (code === "weekly_reminder_limit") return locale === "ru" ? "Достигнут лимит: 10 напоминаний за 7 дней." : locale === "lv" ? "Sasniegts limits: 10 atgādinājumi 7 dienās." : "The limit of 10 reminders in 7 days has been reached.";
  return null;
}
...
export function CalendarPanel({ open, locale, preferences, onClose }: Props) {
  ...
  const quotaBlocked = state ? reminderQuotaBlocked(state.reminderQuota) : true;
  const canChooseReminder = state?.availability === "available" && (!quotaBlocked || Boolean(editing?.reminder));
  ...
  const openNew = (date = selectedDate) => { setSelectedDate(date); setEditing(null); setDraft(emptyDraft(date, state?.preference.timezone ?? "Europe/Riga", state?.availability === "available" && !quotaBlocked)); setConsentChecked(false); setError(null); };
  ...
  const submit = async () => {
    ...
    try {
      if (reminder !== null && !state?.preference.consentAt) {
    ...
    } catch (saveError) { const code = saveError instanceof CalendarRequestError ? saveError.code : ""; setError(code === "event_conflict" ? t.conflict : quotaError(locale, code) ?? t.saveError); }
