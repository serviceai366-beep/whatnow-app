...
function emptyDraft(date: string, preferences: UserProfilePreferences): Draft { return { title: "", date, time: "09:00", allDay: false, timezone: "Europe/Riga", location: "", notes: "", reminder: String(preferences.defaultReminderMinutes) as Draft["reminder"] }; }
...
export function CalendarPanel({ open, locale, preferences, onClose }: Props) {
  ...
  const [draft, setDraft] = useState<Draft>(() => emptyDraft(iso(new Date()), preferences));
  ...
  const openNew = () => { setEditing(null); setDraft(emptyDraft(selectedDate, preferences)); setError(null); };
  const openEdit = (event: CalendarEvent) => { setEditing(event); setDraft(draftFromEvent(event)); setError(null); };
  const submit = async () => {
    if (!draft.title.trim()) return;
    setBusy(true); setError(null);
    const reminder = draft.allDay || draft.reminder === "none" ? null : Number(draft.reminder) as ReminderOffsetMinutes;
  ...
  return <div className="hub-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="calendar-panel" role="dialog" aria-modal="true" aria-labelledby="calendar-title">
      ...
      <div className="calendar-layout">
        <div className="calendar-month" aria-label={t.title}><div className="calendar-weekdays">{weekdayNames.map((name) => <span key={name}>{name}</span>)}</div><div className="calendar-grid">{days.map((day) => { const key = iso(day); const events = byDate.get(key) ?? []; const outside = day.getMonth() !== month.getMonth(); return <button type="button" key={key} className={`${key === selectedDate ? "selected " : ""}${outside ? "outside" : ""}`} onClick={() => setSelectedDate(key)} aria-pressed={key === selectedDate}><span>{day.getDate()}</span>{events.length > 0 && <small>{events.length}</small>}</button>; })}</div></div>
