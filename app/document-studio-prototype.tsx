...
const text = {
  en: {
    ...
    readiness: "Information completeness", readinessHint: "Based on the details already provided and the important facts still missing.", green: "Ready for a strong draft", yellow: "Useful, but more detail is recommended", red: "Critical facts are still missing", critical: "Required before a reliable result", helpful: "Helpful for a more complete document",
  ...
  ru: {
    ...
    readiness: "Полнота информации", readinessHint: "Показатель учитывает уже введённые данные и важные сведения, которых ещё не хватает.", green: "Данных достаточно для сильного документа", yellow: "Можно продолжать, но детали улучшат результат", red: "Критически важных фактов не хватает", critical: "Нужно для надёжного результата", helpful: "Поможет сделать документ более полным",
  ...
  lv: {
    ...
    readiness: "Informācijas pilnīgums", readinessHint: "Rādītājs ņem vērā ievadīto informāciju un svarīgos faktus, kuru vēl trūkst.", green: "Pietiek datu kvalitatīvam dokumentam", yellow: "Var turpināt, bet detaļas uzlabos rezultātu", red: "Trūkst kritiski svarīgu faktu", critical: "Vajadzīgs uzticamam rezultātam", helpful: "Palīdzēs izveidot pilnīgāku dokumentu",
...
function ReadinessSummary({ t, readiness, score, missing, onPick, compact = false }: { t: Copy; readiness: Readiness; score: number; missing: { critical: { key: string; label: string }[]; helpful: { key: string; label: string }[] }; onPick: (key: string) => void; compact?: boolean }) {
  return <div className={`studio-readiness ${readiness}${compact ? " compact" : ""}`} aria-live="polite"><div className="studio-readiness-score" aria-label={`${t.readiness}: ${score}%`}><span>{score}%</span><small>{t.readiness}</small></div><div><strong>{t[readiness]}</strong><p>{t.readinessHint}</p>{missing.critical.length > 0 && <MissingList title={t.critical} items={missing.critical} onPick={onPick} />}{missing.helpful.length > 0 && <MissingList title={t.helpful} items={missing.helpful.slice(0, 8)} onPick={onPick} />}</div></div>;
}
