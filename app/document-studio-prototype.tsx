...
const text = {
  en: {
    ...
    create: "Create new", improve: "Improve existing", review: "Review & check", preSign: "Before signing", preSignTitle: "Final pre-signature check", preSignIntro: "Upload or paste the agreement for a last practical review before either side signs.", preSignGenerate: "Run final check", preSignChecks: "We will check for blanks, dates, amounts, parties, contradictions, unusual terms, and questions for the other side.", preSignDone: "Final pre-signature review", templates: "Choose a starting point", details: "Complete the guided brief",
  ...
  ru: {
    ...
    create: "Создать новый", improve: "Улучшить готовый", review: "Проверить документ", preSign: "Перед подписью", preSignTitle: "Финальная проверка перед подписью", preSignIntro: "Загрузите или вставьте договор для последней практической проверки перед подписанием.", preSignGenerate: "Провести финальную проверку", preSignChecks: "Проверим пустые места, даты, суммы, стороны, противоречия, необычные условия и вопросы для второй стороны.", preSignDone: "Финальная проверка перед подписью", templates: "Выберите основу", details: "Заполните понятную анкету",
  ...
  lv: {
    ...
    create: "Izveidot jaunu", improve: "Uzlabot esošo", review: "Pārskatīt dokumentu", preSign: "Pirms parakstīšanas", preSignTitle: "Pēdējā pārbaude pirms parakstīšanas", preSignIntro: "Augšupielādējiet vai ielīmējiet līgumu pēdējai praktiskai pārbaudei pirms parakstīšanas.", preSignGenerate: "Veikt pēdējo pārbaudi", preSignChecks: "Pārbaudīsim tukšās vietas, datumus, summas, puses, pretrunas, neparastus nosacījumus un jautājumus otrai pusei.", preSignDone: "Pēdējā pārbaude pirms parakstīšanas", templates: "Izvēlieties sākumpunktu", details: "Aizpildiet vadīto informāciju",
...
export function DocumentStudioPrototype({ locale, account, onRequireAccount, onOpenPlan }: { locale: ProfileLanguage; account: SupabaseAccount | null; onRequireAccount: () => void; onOpenPlan: () => void }) {
  ...
  const [preSignatureCheck, setPreSignatureCheck] = useState(false);
  ...
  return <section className="studio-shell" aria-labelledby="studio-title">
    ...
    <div className="studio-action-switch" role="tablist">{(["create", "improve", "review"] as StudioMode[]).map((value) => <button key={value} type="button" role="tab" aria-selected={mode === value && !preSignatureCheck} className={mode === value && !preSignatureCheck ? "active" : ""} onClick={() => { setMode(value); setPreSignatureCheck(false); setFields({}); setSourceFile(null); setAssistantMessages([]); }}>{t[value]}</button>)}<button type="button" role="tab" aria-selected={preSignatureCheck} className={preSignatureCheck ? "active pre-sign" : "pre-sign"} onClick={() => { setMode("review"); setPreSignatureCheck(true); setFields({}); setSourceFile(null); setAssistantMessages([]); }}>{t.preSign}</button></div>
    {workflow === "guided" ? <div className="studio-workspace">
      ...
      <div className="studio-form-panel">
        <div className="studio-form-title"><span className="studio-step">{preSignatureCheck ? "✓" : "2"}</span><div><h2>{preSignatureCheck ? t.preSignTitle : t.details}</h2><p>{preSignatureCheck ? t.preSignIntro : mode === "create" ? guideText(guide.intro, copyLocale) : t.goalHint}</p></div></div>
        ...
        <button className="primary-button studio-generate" disabled={busy || quota.remaining === 0} type="button" onClick={() => void generate()}>{busy ? t.generating : preSignatureCheck ? t.preSignGenerate : t.generate}<span>→</span></button>
      ...
      <div className="studio-quick-card">
        <div className="studio-form-title"><span className="studio-step">{preSignatureCheck ? "✓" : "⚡"}</span><div><h2>{preSignatureCheck ? t.preSignTitle : t.quickTitle}</h2><p>{preSignatureCheck ? t.preSignIntro : t.quickIntro}</p></div></div>
        ...
        {preSignatureCheck ? <div className="studio-quick-warning pre-sign"><span>✓</span><div><strong>{t.preSign}</strong><p>{t.preSignChecks}</p></div></div> : <div className="studio-quick-warning"><span>!</span><div><strong>{t.quickAccuracy}</strong><p>{t.quickAccuracyBody}</p></div></div>}
        {error && <p className="studio-api-error" role="alert">{error}</p>}
        <button className="primary-button studio-generate" disabled={busy || quota.remaining === 0 || quickPrompt.trim().length < 2} type="button" onClick={() => void generate()}>{busy ? t.generating : preSignatureCheck ? t.preSignGenerate : t.quickGenerate}<span>→</span></button>
        {busy && <div className="studio-generation-progress" role="status" aria-live="polite"><p className="studio-generation-timer"><span>{t.reasoningTime}: {formatDuration(elapsed)} / 10:00</span><small>{t.reasoningEstimate}</small></p><button type="button" onClick={cancelGeneration}>{t.stop}</button></div>}
      </div>
