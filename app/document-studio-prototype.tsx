...
const text = {
  en: {
    live: "Secure AI workspace", title: "Create & edit documents", subtitle: "A guided Pro workspace for near-final, jurisdiction-aware documents.",
    ...
    back: "Back to details", draft: "Working document", copy: "Copy", docx: "DOCX", pdf: "PDF", history: "Recent documents", historyButton: "Document history", empty: "Your latest 10 generated documents will appear here.", limit: "Pro usage", delete: "Delete", sources: "Official sources consulted", issues: "Check before use", error: "The document could not be generated. Try again.",
  ...
  lv: {
    ...
    back: "Atpakaļ pie datiem", draft: "Darba dokuments", copy: "Kopēt", docx: "DOCX", pdf: "PDF", history: "Jaunākie dokumenti", historyButton: "Dokumentu vēsture", empty: "Šeit būs pēdējie 10 dokumenti.", limit: "Pro lietojums", delete: "Dzēst", sources: "Pārbaudītie oficiālie avoti", issues: "Pārbaudīt pirms lietošanas", error: "Dokumentu neizdevās izveidot. Mēģiniet vēlreiz.",
...
export function DocumentStudioPrototype({ locale, account, onRequireAccount, onOpenPlan }: { locale: ProfileLanguage; account: SupabaseAccount | null; onRequireAccount: () => void; onOpenPlan: () => void }) {
  ...
  const generate = async (confirmed = false) => {
    ...
    if (quota?.planCode !== "pro") { onOpenPlan(); return; }
  ...
  const askPreparationAssistant = async (preset?: string) => {
    const question = (preset ?? assistantQuestion).trim(); if (!question || !account || quota?.planCode !== "pro") return;
