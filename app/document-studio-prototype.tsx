...
const text = {
  en: {
    ...
    upload: "Or attach the existing document", chooseFile: "Choose document", scanDocument: "Scan with camera", removeFile: "Remove", fileHint: "PDF, image, TXT, RTF, DOCX, or ODT. Maximum size depends on format.", reasoningTime: "Reasoning time", reasoningEstimate: "Usually about 2–3 minutes, with a 10-minute maximum.",
  ...
  ru: {
    ...
    upload: "Или прикрепите готовый документ", chooseFile: "Выбрать документ", scanDocument: "Сканировать камерой", removeFile: "Удалить", fileHint: "PDF, изображение, TXT, RTF, DOCX или ODT. Максимальный размер зависит от формата.", reasoningTime: "Время рассуждения", reasoningEstimate: "Обычно около 2–3 минут, максимум — 10 минут.",
  ...
  lv: {
    ...
    upload: "Vai pievienojiet esošo dokumentu", chooseFile: "Izvēlēties dokumentu", scanDocument: "Skenēt ar kameru", removeFile: "Noņemt", fileHint: "PDF, attēls, TXT, RTF, DOCX vai ODT. Maksimālais izmērs atkarīgs no formāta.", reasoningTime: "Spriešanas laiks", reasoningEstimate: "Parasti ap 2–3 minūtēm, maksimums 10 minūtes.",
...
export function DocumentStudioPrototype({ locale, account, onRequireAccount, onOpenPlan, onOpenScanner }: { locale: ProfileLanguage; account: SupabaseAccount | null; onRequireAccount: () => void; onOpenPlan: () => void; onOpenScanner: () => void }) {
  ...
  return <section className="studio-shell" aria-labelledby="studio-title">
    ...
    {workflow === "guided" ? <div className="studio-workspace">
      ...
      <div className="studio-form-panel">
        ...
        {mode !== "create" && <StudioFileField t={t} file={sourceFile} onFile={(file) => { setSourceFile(file); setError(""); }} onScan={onOpenScanner} />}
...
function StudioFileField({ t, file, onFile, onScan }: { t: Copy; file: File | null; onFile: (file: File | null) => void; onScan?: () => void }) {
  return <div className="studio-file-field"><span>{t.upload}</span><input key={file ? `${file.name}-${file.lastModified}` : "empty"} id="studio-document-file" className="visually-hidden" type="file" accept=".pdf,.jpg,.jpeg,.png,.webp,.txt,.rtf,.docx,.odt" onChange={(event) => onFile(event.target.files?.[0] ?? null)} /><div className="studio-file-actions"><label htmlFor="studio-document-file" className="studio-file-button">{t.chooseFile}</label>{onScan && <button type="button" className="studio-file-scan" onClick={onScan}><span aria-hidden="true">⌾</span> {t.scanDocument}</button>}</div>{file && <div className="studio-selected-file"><strong title={file.name}>{file.name}</strong><span>{Math.max(1, Math.round(file.size / 1024))} KB</span><button type="button" onClick={() => onFile(null)}>{t.removeFile}</button></div>}<small>{t.fileHint}</small></div>;
}
